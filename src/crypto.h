...
byte* nullable cryptoAddPadding(unsigned* newSize, const byte* bytes, unsigned size); // returns null if no padding needed (size % BLOCK_SIZE == 0) - use original input; newSize is either grown size or original (if result is null)
